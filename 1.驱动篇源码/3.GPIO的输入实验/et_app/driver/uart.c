...
void ICACHE_FLASH_ATTR
uart_init_new(UART_BautRate baud_rate, et_uchar fac_norm_mode)
{
	et_int32 ret = -1;
	...
	if(ret == 0)
	{
	    ...
	    if(fac_norm_mode != FAC_MODE)    
	    {
	    	xTaskCreate(read_uart_buf_task, (uint8 const *)"uTask", 1024, NULL, tskIDLE_PRIORITY + 2, &xUartTaskHandle);
	    } 
	    else
	    {
	    	xTaskCreate(fac_read_uart_task, (uint8 const *)"uTask", 1024, NULL, tskIDLE_PRIORITY + 4, &xUartTaskHandle);
	    }
	}
	else
	{
