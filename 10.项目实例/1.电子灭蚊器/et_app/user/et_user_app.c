...
et_int32 parse_msg_from_mqtt(et_uchar *msg_buf, et_int32 data_len)
{
	...
	else if(strcmp(msg_buf,my_get)==0)
	{
		if(my_gpio==0)
			rc = et_chat_to(g_cloud_handle,"设备正在运行", 10, g_user_id, SEND_TO_CLOUD_FIRST);
		else if(my_gpio==1)
			rc = et_chat_to(g_cloud_handle,"设备已经关闭", 10, g_user_id, SEND_TO_CLOUD_FIRST);
	}
	else
	{
